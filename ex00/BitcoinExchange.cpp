...
void BitcoinExchange::mapDatabase() {
	...
	if (dbFile.bad()) {
		std::string err_msg =
			"Error while reading file " + filename + " " + std::strerror(errno);
...
void BitcoinExchange::processInput(std::string file) {
	...
	while (std::getline(inputFile, line)) {
		...
		if (_validateDate(date) == false ||
			_validateValue(valueStr, value) == false)
			continue;
	...
	if (inputFile.bad()) {
		std::string err_msg = ERROR "Error while reading file " + file + " " +
							  std::strerror(errno);
		throw std::runtime_error(err_msg);
	}
}
...
void BitcoinExchange::_checkAndSkipHeader(std::string filename,
...
bool BitcoinExchange::_checkDayWithinMonth(long day, long month, long year) {
	static const short daysPerMonth[12] = {31, 28, 31, 30, 31, 30,
										   31, 31, 30, 31, 30, 31};
	short month_idx = static_cast<short>(month - 1);

	if (month_idx != 1 && day > daysPerMonth[month_idx]) {
		std::cout << ERROR << "day (" << day << ") out of range for month "
				  << month << '\n';
		return false;
	} else if (month_idx == 1) {
		bool isLeapYear =
